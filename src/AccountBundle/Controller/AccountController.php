...
class AccountController extends Controller
{
     ...
     * @Method({"GET", "POST"})
     */
    public function assginBeneficiary(Request $request, Account $account)
    {
        ...
        if ($assignForm->isSubmitted() && $assignForm->isValid()) {
            ...
            $beneficiary = $assignForm->get('beneficiary')->getData();
            foreach ($beneficiary as $beneficiaryy) {
              $account->addBeneficiary($beneficiaryy);
            }
            $this->getDoctrine()->getManager()->flush();
            return $this->redirectToRoute('account_show', array('id' => $account->getId()));
        }

        return $this->render('account/assignBeneficiary.html.twig', array(
            'account' => $account,
    ...
    public function transactionBeneficiary(Request $request, Account $account)
    {
        ...
        return $this->render('account/transactionBeneficiary.html.twig', array(
            'account' => $account,
